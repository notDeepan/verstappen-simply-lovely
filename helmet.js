...
export const HELMET_MODEL = {
  ...
  embed: 'https://sketchfab.com/models/f7b576c4294441478b3ffe3900360de1/embed?autostart=1&dnt=1&ui_infos=0&ui_inspector=0&ui_theme=dark',
  author: 'bad_bovy',
};
...
function loadViewerAPI() {
  ...
  viewerScript ??= new Promise((resolve, reject) => {
    ...
    script.onerror = () => { script.remove(); viewerScript = undefined; reject(new Error('Viewer API unavailable')); };
...
export function createHelmetStudio(container) {
  ...
  let loadingTimer;
  let generation = 0;
  const fallback = container.closest('dialog').querySelector('.helmet-viewer-fallback');
  ...
  function render() {
    ...
    frame.title = 'Max Verstappen 2024 Imola helmet — interactive 3D by bad_bovy on Sketchfab';
    ...
    loadViewerAPI().then(Sketchfab => {
      ...
      new Sketchfab('1.12.1', frame).init(HELMET_MODEL.id, {
        autostart: 1, dnt: 1, ui_infos: 0, ui_inspector: 0, ui_theme: 'dark',
        success(api) {
          if (current !== generation) return;
  ...
  return {
    resume() { active = true; render(); },
    pause() { active = false; unload(); },
    setReference(value) { referenceOnly = value; render(); },
    reset() { referenceOnly = false; unload(); render(); },
    dispose() { active = false; unload(); },
  };
}
